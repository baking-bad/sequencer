...
fn main() {
    ...
    cmd.args(vec![
        "run-comb",
        "--primary-keys", format!("./launcher/defaults/primary-{}.key", args.id).as_str(),
        "--primary-network-keys", format!("./launcher/defaults/primary-network-{}.key", args.id).as_str(),
        "--worker-keys", format!("./launcher/defaults/worker-network-{}.key", args.id).as_str(),
        "--committee", "./launcher/defaults/committee.json",
        "--workers", "./launcher/defaults/workers.json",
        "--primary-store", format!("./.db/primary-{}", args.id).as_str(),
        "--worker-store", format!("./.db/worker-{}", args.id).as_str(),
    ]);
